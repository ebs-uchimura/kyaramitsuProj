  ...
  selectDB = async (args: selectargs): Promise<any> => {
    return new Promise(async (resolve) => {
      try {
        ...
        // order (optional)
        const order1: any = args.order1 ?? null;
        // order (optional)
        const order2: any = args.order2 ?? null;
        // order (optional)
        const order3: any = args.order3 ?? null;
        // limit (optional)
        ...
        if (order2) {
          queryString += ', ?? DESC';
          // push order key
          placeholder.push(order2);
        }
        // if order exists
        if (order3) {
          queryString += ', ?? DESC';
          // push order key
          placeholder.push(order3);
        }

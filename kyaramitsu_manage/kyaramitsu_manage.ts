...
// フォルダ作成
const mkdirManager: MKDir = new MKDir(logger);
// express設定
...
app.get('/shop_edit', isAdminAuthenticated, async (req: any, res: any) => {
  try {
    ...
    // 都道府県
    if (prefectureid) {
      tmpColumns = ['prefecture_id', 'closed', 'usable'];
      tmpValues = [[prefectureid], [0], [1]];
      // なし
    } else {
      tmpColumns = ['closed', 'usable'];
      tmpValues = [[0], [1]];
    }
    // 対象データ
...
app.get('/editpage', isAdminAuthenticated, async (req: any, res: any) => {
  try {
    ...
    let shopSelectArgs: selectargs = {
      table: 'shop', // テーブル
      columns: ['id', 'closed', 'usable'], // カラム
      values: [[Number(shopid)], [0], [1]], // 値
      limit: 1, // 上限
    };
    // 対象店舗データ取得
    const targetShopData: any = await myDB.selectDB(shopSelectArgs);
...
app.post('/regist', isAdminAuthenticated, async (req: any, res: any) => {
  try {
    ...
    // 受信データ
    const reqCustomerno: any = req.body.customerno ?? null; // 顧客番号
    const reqShopname: any = req.body.shopname ?? null; // 店舗名
    const reqHeadline: any = req.body.headline ?? null; // 見出し
    ...
    const reqZip: any = req.body.zip ?? null; // 郵便番号
    const reqAddress: any = req.body.address ?? null; // 住所
    const reqCityname: any = req.body.cityname ?? null; // 認定エリア名
    ...
    const insertShopArgs: insertargs = {
      ...
      values: [
        areaCode, // エリアID
        sanitizeHtml(reqShopname), // 店舗名
        sanitizeHtml(reqHeadline), // 見出し
        sanitizeHtml(reqDetail), // 店舗詳細
        prefCode, // 都道府県
        Number(sanitizeHtml(reqCustomerno)), // 顧客番号
        sanitizeHtml(reqCityname), // 認定エリア名
        reqStartTime1, // 営業開始時間1
        reqEndTime1, // 営業終了時間1
        reqStartTime2, // 営業開始時間2
        reqEndTime2, // 営業終了時間2
        sanitizeHtml(reqZip), // 郵便番号
        sanitizeHtml(reqAddress), // 住所
        sanitizeHtml(reqHoliday), // 定休日
        sanitizeHtml(reqPathFile1), // 画像1
        sanitizeHtml(reqPathFile2), // 画像2
        sanitizeHtml(reqPathFile3), // 画像3
        sanitizeHtml(reqPathFile4), // 画像4
        sanitizeHtml(reqPathFile5), // 画像5
        0, // 閉店
        1, // 表示
        1 // 使用可能
      ]
...
app.post('/edit', isAdminAuthenticated, async (req: any, res: any) => {
  try {
    logger.info('manage: shop edit posted');
    // 変数
    let tmpClosed: number; // 閉店
    let tmpDisplay: number; // 表示
    let tmpUseOriginal: number; // オリジナル画像
    // 受信データ
    const reqShopID: any = req.body.id ?? null; // 店舗ID
    ...
    const closedIndex: any = req.body.closed ?? []; // 閉店
    ...
    // サニタイズ後住所
    const sanitizedAddress: any = sanitizeHtml(reqAddress);
    // 都道府県抽出
    const prefecturename: string = sanitizedAddress.match(/..+?[都道府県]/);
    // ヒットなし
    if (!prefecturename) {
      throw new Error('regist: 都道府県が含まれていません');
    }
    ...
    // 閉店判定
    if (closedIndex.length > 0 && closedIndex[0] == String(reqShopID)) {
      tmpClosed = 1;
    } else {
      tmpClosed = 0;
    }
    ...
    const tmpShopValues: any[] = [
      ...
      sanitizeHtml(reqHeadline), // 見出し
      sanitizeHtml(reqDetail), // 店舗詳細
      sanitizeHtml(reqDetailSource), // 店舗詳細文章出典
      sanitizeHtml(reqGenre), // ジャンル
      sanitizeHtml(reqCityname), // 認定エリア名
      reqStartTime1, // 営業開始時間1
      reqEndTime1, // 営業終了時間1
      reqStartTime2, // 営業開始時間2
      reqEndTime2, // 営業終了時間2
      sanitizeHtml(reqBAnnotation), // 営業時間補足
      sanitizeHtml(reqZip), // 郵便番号
      sanitizedAddress, // 住所
      sanitizeHtml(reqHoliday), // 定休日
      sanitizeHtml(reqHAnnotation), // 定休日補足
      sanitizeHtml(reqPathFile1), // 画像1
      sanitizeHtml(reqPathFile2), // 画像2
      sanitizeHtml(reqPathFile3), // 画像3
      sanitizeHtml(reqPathFile4), // 画像4
      sanitizeHtml(reqPathFile5), // 画像5
      sanitizeHtml(reqOriginalImg), // オリジナル画像
      sanitizeHtml(reqImgSource), // 画像出典
...
app.post('/delete', isAdminAuthenticated, async (req: any, res: any) => {
  try {
    ...
    if (!reqShopID) {
      // 店舗名なしエラー
      throw new Error('delete: no necessary data error');
    }
    // カラム
...
// 画像保存
const savImageFile = async (file: any, column: string, shopid: number): Promise<void> => {

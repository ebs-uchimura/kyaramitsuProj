...
app.get('/', async (_: any, res: any) => {
  try {
    ...
    // 全店舗カウント
    const shopAreaCountArgs: countargs = {
      table: 'shop', // テーブル
    ...
    // 全体初期リスト表示
    const shopAreaSelectArgs: selectargs = {
      table: 'shop', // テーブル
      columns: ['display', 'usable'], // カラム
      values: [[1], [1]], // 値
      order1: 'useoriginal',
...
app.get('/area', async (req: any, res: any) => {
  try {
    ...
    // カラム
    res.redirect(`${myConst.DEFAULT_URL}area/1/1`);
...
app.get('/area/:areaid', async (req: any, res: any) => {
  try {
    logger.debug('top: area started');
    // エリアID
    const areaid: string = req.params.areaid;
    // カラム
    res.redirect(`${myConst.DEFAULT_URL}area/${areaid}/1`);
...
app.get('/area/:areaid/:page', async (req: any, res: any) => {
  try {
    ...
    // 対象店舗データ取得
    const shopCount: any = await myDB.countDB(shopAreaCountArgs);
    // 全体初期リスト表示
    const shopAreaSelectArgs: selectargs = {
      table: 'shop', // テーブル
      columns: tmpColumns, // カラム
...
app.get('/pref/:prefid/:page', async (req: any, res: any) => {
  try {
    ...
    const shopCount: number = await myDB.countDB(shopAreaCountArgs);
    // 対象データ
    const shopPrefSelectArgs: selectargs = {
      table: 'shop', // テーブル
      columns: ['prefecture_id', 'display', 'usable'], // カラム
      values: [[Number(prefid)], [1], [1]], // 値
      offset: pageOffset, // 開始
...
app.get('/shop/:shopid', async (req: any, res: any) => {
  try {
    ...
    // 対象データ
    const shopJoinSelectArgs: joinargs = {
      table: 'shop', // テーブル
    ...
    // 対象店舗データ取得
    const selectedShopData: any = await myDB.selectJoinDB(shopJoinSelectArgs);
    // 結果
    if (selectedShopData == 'error') {
      // DBエラー
      throw new Error('mysql: selectShop error');
      ...
      // 都道府県
      const prefSelectArgs: selectargs = {
        table: 'prefecture', // テーブル
        columns: ['id', 'usable'], // カラム

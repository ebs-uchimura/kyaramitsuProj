...
app.get('/shop_edit', isAdminAuthenticated, async (req: any, res: any) => {
  try {
    ...
    let tmpValues: any[];
    // エリア
    const areaid: any = req.query.area;
    // 都道府県
    const prefectureid: any = req.query.prefecture;
    // エリア
    if (areaid) {
      tmpColumns = ['area_id', 'usable'];
      tmpValues = [areaid, 1];
      // 都道府県
...
app.post('/regist', isAdminAuthenticated, async (req: any, res: any) => {
  try {
    ...
    // 登録アリ
    if (!reqShopname || !reqCityname || !reqHeadline || !reqZip || !reqAddress) {
      // 店舗名なしエラー
